...
- RDAP, generic WHOIS, UK, JP, FR, CN, BR, IT, EU, BE, PL, CZ, HU, SK, RO, DE, NL, CA, AU, SE/NU, FI, KR, AT, RU/SU, EE, BG, KG, TR, HK, TW, SI, UA, ID, KZ, MX, PT, QA, MD, LU, LV, AM, AX, BD, BN, GG/JE, IL, LT, MO, RS, ST, TN, and UZ WHOIS parsers feed a unified merger, with golden fixtures.
- WHOIS Web fallback is a separate provider plugin, not a runtime dependency on `unofficial/`; it currently has manual notice modules for selected TLDs plus rewritten stable modules for `.dz`, `.lk`, `.mt`, `.ni`, `.pa`, and `.vn`.
...
- Optional registrar and nameserver brand enrichment exists with a mounted `data/brands/brand-map.json` override and embedded snapshot derived from Whoice defaults plus `unofficial/next-whois` UI-branding ideas. Brand metadata can include color, logo, website, and aliases, and the Web UI falls back gracefully when a logo is absent.
...
- `pnpm test:data` validates data manifests, hash integrity, parser/readability of data files, brand-map structure, embedded snapshot sync, and critical PSL/WHOIS routing coverage.
