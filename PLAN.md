...
当前迁移进度：已从 `unofficial/whois-domain-lookup` 的 TLD parser 思路重写 UK、JP、FR、CN、BR、IT、EU、BE、PL、CZ、HU、SK、RO，并为每个新增 parser 放入 golden fixture。后续继续按“高价值 TLD + 真实样本 + schema fixture 校验”的节奏迁移。
...
```txt
WHOICE_PUBLIC_BASE_URL=https://whoice.example.com
WHOICE_API_ADDR=:8080
WHOICE_WEB_PORT=18081
WHOICE_WEB_API_BASE=http://lookup-api:8080

WHOICE_SITE_TITLE=Whoice
WHOICE_SITE_DESCRIPTION=Modern WHOIS/RDAP lookup

WHOICE_AUTH_MODE=none|password|token
WHOICE_SITE_PASSWORD=
WHOICE_API_TOKENS=

WHOICE_DATA_DIR=/data
WHOICE_TRUST_PROXY=false
WHOICE_METRICS_ENABLED=true

WHOICE_LOOKUP_TIMEOUT=15s
WHOICE_PROVIDER_TIMEOUT=10s
WHOICE_RDAP_ENABLED=true
WHOICE_WHOIS_ENABLED=true
WHOICE_WHOIS_WEB_ENABLED=true
WHOICE_WHOIS_FOLLOW_LIMIT=1
WHOICE_ALLOW_CUSTOM_SERVERS=false
WHOICE_ALLOW_PRIVATE_SERVERS=false
...
WHOICE_PRICING_PROVIDER=nazhumi
WHOICE_ENRICH_MOZ=false
WHOICE_MOZ_ACCESS_ID=
WHOICE_MOZ_SECRET_KEY=
...
自部署用户需要简单配置；高级功能需要能独立打开/关闭。

### 当前最佳方案

环境变量 + 可选 YAML 配置：

- Docker/Vercel/CI 用环境变量。
- 复杂部署可挂载 `config.yaml`。
...
- Docker 对外 Web：`${WHOICE_WEB_BIND:-0.0.0.0:18081}`。
- Docker 对外 API：`${WHOICE_API_BIND:-0.0.0.0:18080}`。
...
需要本地改 Dockerfile 或离线构建时，才叠加 `deploy/compose/docker-compose.build.yml`。

### 9.1.1 多架构镜像发布

每次发布版本 tag，例如当前 alpha 的 `0.01alpha` 或未来稳定版 `v0.1.0`，GitHub Actions 自动用 Buildx 构建并推送：
...
- `data/rdap-bootstrap/*.json` 覆盖 RDAP bootstrap。
- `data/registrars/icann-accredited-registrars.csv` 覆盖 ICANN registrar metadata。
- `data/brands/brand-map.json` 覆盖 registrar/NS brand map。
- `data/whois-servers/iana.json` 与 `data/whois-servers/extra.json` 覆盖 WHOIS server map。
...
| RDAP/WHOIS 上游限流 | 查询失败 | cache + rate limit + user-agent + timeout |
